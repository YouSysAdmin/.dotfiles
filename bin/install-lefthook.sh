...
usage() {
  this=$1
  cat <<EOF
$this: download go binaries for evilmartians/lefthook
...
hash_sha256_verify() {
  ...
  sum=$(cat "${checksums}" | grep "${TARBALL}")
  want=$(echo ${sum% *})
  if [ -z "$want" ]; then
...
TARREALTAG=$(echo $REALTAG | sed 's/v//')
NAME=${PROJECT_NAME}_${TARREALTAG}_${OS}_${ARCH}
TARBALL=${NAME}.${FORMAT}
TARBALL_URL=${GITHUB_DOWNLOAD}/${REALTAG}/${TARBALL}
CHECKSUM_FILE_NAME="lefthook_checksums.txt"
